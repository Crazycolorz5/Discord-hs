...
data Message = Message {message_id :: Snowflake, channel_id :: Snowflake, author :: Maybe User, content :: String, timestamp :: Timestamp, edited_time :: Maybe Timestamp, tts :: Bool, mention_everyone :: Bool, mentions :: [User], mention_roles :: [Role], attachments :: [Attachment], embeds :: [Embed], reactions :: [Reaction], message_nonce :: Maybe Snowflake, pinned :: Bool, webhook_id :: String}
...
parseJSON = do
    ...
    parseObject = do
        begin_object
        contents <- parseMember <&> kleeneStar (value_separator >> parseMember)
        end_object
        return $ JSONObj (process contents) where
            
          process (mem, mems) = uncurry Map.insert mem (Map.fromList mems)
